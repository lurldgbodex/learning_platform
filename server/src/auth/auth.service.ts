...
export class AuthService {
   ...
   async refreshToken(refreshTokenDto: RefreshTokenDto): Promise<AuthResponse> {
    ...
    try {
        ...
        const newPayload = { email: user.email, sub: payload.sub, role: payload.role };
   ...
   private async generateToken(payload: TokenPayload): Promise<AuthResponse> {
    const accessToken = await this.jwtService.signAsync(payload, { secret: jwtConstants.secret, expiresIn: '1h' });
